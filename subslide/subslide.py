...
from patch_reader import sample_store_patches, sample_store_boundary_patches
...
class Cutter(object):
    def __init__(self, 
        ...
        Params:
            ...
            mask_dir: tissue mask path
            anno_dir: annotation file path
            save_dir: save file path
            target_dir: target file path
            label_map: dictionary of labels and integer labels
        ...
        print("Full Mask directory:       ", self.full_mask_dir)
        print("Data store directory:      ", self.save_dir)
        print("Target store directory:    ", self.target_dir)
    ...
    def sample_and_store_patches(self,
        ...
        for file in tqdm(self.files):
            print(file, end=" ")
            patches_num = sample_store_patches(file,
                                                self.file_dir,
                                                self.mask_dir,
                                                self.anno_dir,
                                                self.save_dir,
    ...
    def sample_and_store_boundary_patches(self,
                                            patch_size,
                                            level,
                                            filter_rate,
                                            resize_factor=2,
                                            ):
        """ Samples boundary patches from all whole slide images in the dataset and stores them in the
            specified format.
            - patch_size        the patch size in pixels to sample
            - level             the tile level to sample at
        """
        ...
        for file in tqdm(self.files):
            print(file, end=" ")
            patches_num = sample_store_boundary_patches(file,
                                                        self.file_dir,
                                                        self.full_mask_dir,
                                                        self.save_dir,
                                                        self.target_dir,
